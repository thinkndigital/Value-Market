...
| `CartService::addToCart()` returned `false` for a brand-new cart item (only returned `true` for `$fromApp=true` or an already-existing row), and `return`ed from *inside* its loop the moment it processed any item — so a multi-item cart only ever touched its first line, and a walk-in customer's first purchase of any item failed with "Items are Not Added" | **Fixed** — 32-phase brief Phase 9 (`docs/PHASE_9_POS_CART_FIX.md`). Proven by `PosSaleTest`/`CartServiceMultiItemTest`. | `PHASE_1_TRANSACTION_BOUNDARIES.md` §2 |
| `CartService::addToCart()` crashed (`ErrorException: Undefined array key 1`) on any cart with more than one distinct new product — it indexed a single-element `store_id` array per item | **Fixed** — same pass, same doc. A missing per-item index now falls back to the first (shared) value instead of crashing, without changing the genuine per-item multi-vendor-cart case (`CartServiceMultiItemTest`). | `PHASE_1_TRANSACTION_BOUNDARIES.md` §2 |
| POS `place_order()`'s order-item creation loop returns after its first iteration — multi-item POS carts (that get past the `addToCart` crash above) silently drop all but one line item | **Fixed** — Phase 6 (`docs/PHASE_6_POS.md` §1), predating this entry going stale here. Proven by `PosMultiItemAndPaymentsTest`. | `PHASE_1_TRANSACTION_BOUNDARIES.md` §2 |
| POS `place_order()` never decrements stock for regular products | **Fixed** — Phase 6 (`docs/PHASE_6_POS.md` §2), predating this entry going stale here. Proven by `PosSaleTest`/`PosComboStockTest`. | `PHASE_1_TRANSACTION_BOUNDARIES.md` §2 |
...
| Money display formatting is currency-blind | **Fixed** — 32-phase brief Phase 15. Re-verified before fixing: only `formatePriceDecimal()` actually hardcoded 2 decimals (`number_format($price, 2)`); `CurrencyService::formateCurrency()` never touched decimal precision at all — it just concatenates a currency symbol onto whatever string `formatePriceDecimal()` already produced (confirmed by reading every one of its ~20 call sites, which all call `formateCurrency(formatePriceDecimal($price))`). Fixed with a static ISO 4217 minor-unit exception table (`CurrencyService::decimalPlacesFor()` — 0dp for JPY/KRW/etc., 3dp for JOD/KWD/BHD/etc., 2dp default), `formatePriceDecimal($price, $currencyCode = null)` resolving the currency code through the app's own configured default when omitted so every existing call site is automatically correct with no changes needed. Proven by `tests/Feature/Phase15/CurrencyDecimalPrecisionTest.php`. |
