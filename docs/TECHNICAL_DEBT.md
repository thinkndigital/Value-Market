...
| `CartService::addToCart()` returns `false` for a brand-new cart item (only returns `true` for `$fromApp=true` or an already-existing row); POS always calls it without `$fromApp=true`, so a walk-in customer's first purchase of any item fails with "Items are Not Added" | **Documented, not fixed** — Phase 6. Confirmed by automated test, not just reading. | `PHASE_1_TRANSACTION_BOUNDARIES.md` §2 |
| `CartService::addToCart()` crashes (`ErrorException: Undefined array key 1`) on any cart with more than one distinct new product — it indexes a single-element `store_id` array per item | **Documented, not fixed** — Phase 6. Confirmed by automated test; this is usually the *first* thing to fail on a genuine multi-item sale, ahead of the loop bug below. | `PHASE_1_TRANSACTION_BOUNDARIES.md` §2 |
| POS `place_order()`'s order-item creation loop returns after its first iteration — multi-item POS carts (that get past the `addToCart` crash above) silently drop all but one line item | **Documented, not fixed** — Phase 6 | `PHASE_1_TRANSACTION_BOUNDARIES.md` §2 |
| POS `place_order()` never decrements stock for regular products | **Documented, not fixed** — Phase 6. Confirmed by automated test. | `PHASE_1_TRANSACTION_BOUNDARIES.md` §2 |
