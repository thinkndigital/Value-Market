...
| 2 | Complete Route & Page Inventory | PARTIAL | `docs/PHASE_2_ROUTE_SWEEP_REPORT.md` — automated sweep of all 298 no-param GET routes across 4 panels (real user, real HTTP kernel). Found 75 real 500s, root-caused every one, fixed the 4 confirmed-safe ones (shared pagination bug), categorized the rest by real-world risk. All 107 param-required routes now triaged across 3 batches: batch 1 (39 admin single-model-CRUD routes) fixed 2 real bugs (`admin/area/edit/{id}` wired to a nonexistent method; a null-array-offset crash on `admin/sellers/edit/{id}`); batch 2 (37 seller/delivery_boy/affiliate routes) found 9 more broken routes, all confirmed unreachable, root-caused and documented, not fixed; batch 3 (31 admin routes needing richer fixtures — products, combo products, orders, settings) fixed 4 real reachable bugs, the biggest being an app-wide crash in every notification-triggering action (order/product status changes, returns, tickets) whenever Firebase push isn't configured, fixed at its single choke point (`FirebaseNotificationService::sendNotification()`), plus 2 more confirmed-unreachable dead routes documented. `tests/Feature/RouteSweepTest.php` + `tests/Feature/Phase2/ParamRouteSweepBatch{1,2,3}Test.php` are permanent regression coverage. Not done: everything past "does it render" (forms/AJAX/search/filters/RTL) per this brief's own fuller checklist. |
...
| 21 | API | PARTIAL | `docs/PHASE_21_API_AUDIT.md` — all 109 GET routes across the three `v1\ApiController` classes swept with real HTTP kernel + real Sanctum tokens + real fixtures (`tests/Feature/Phase21/*RouteSweepTest.php`). Result: the read surface is healthy — only 2 dead routes found (`api/test`, `api/get_phonepe_token`, both confirmed broken with no working equivalent, documented not fixed — no mobile client source available to confirm either is actually called). Not done: the ~97 POST/PUT/DELETE routes (registration, cart, orders, payments, seller/delivery mutations), contract/pagination consistency, and versioning strategy. |
