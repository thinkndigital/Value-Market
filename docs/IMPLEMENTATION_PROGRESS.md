...
| 2 | Complete Route & Page Inventory | PARTIAL | `docs/PHASE_2_ROUTE_SWEEP_REPORT.md` — automated sweep of all 298 no-param GET routes across 4 panels (real user, real HTTP kernel). Found 75 real 500s, root-caused every one, fixed the 4 confirmed-safe ones (shared pagination bug), categorized the rest by real-world risk. Param-route sweep (107 routes total) now fully triaged: batch 1 (39 admin single-model-CRUD routes) fixed 2 real bugs (`admin/area/edit/{id}` wired to a nonexistent method; a null-array-offset crash on `admin/sellers/edit/{id}`); batch 2 (37 seller/delivery_boy/affiliate routes) found 9 more broken routes, all confirmed unreachable (no UI ever links to them) — root-caused and documented, not fixed (real but unscoped feature work). `tests/Feature/RouteSweepTest.php` + `tests/Feature/Phase2/ParamRouteSweepBatch{1,2}Test.php` are permanent regression coverage. Not done: admin routes needing richer multi-model fixtures (products, orders, combo products, attributes), and everything past "does it render" (forms/AJAX/search/filters/RTL) per this brief's own fuller checklist. |
