...
| 6 | Merchant-Specific Payment Gateways | PARTIAL | `docs/PHASE_6_PAYMENT_GATEWAYS.md` — `seller_payment_gateways` table (encrypted-at-rest credentials), resolver service, seller self-service CRUD panel, all fully wired end-to-end through Razorpay (the reference implementation) at both real order/store-aware checkout entry points. Not done: Stripe/Paypal/Paystack/Phonepe not wired yet (each needs its own verify pass — different settings-access shapes), and per-seller webhook routing (today's webhook verification stays platform-global). |
