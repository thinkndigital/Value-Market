...
| 6 | Merchant-Specific Payment Gateways | PARTIAL | `docs/PHASE_6_PAYMENT_GATEWAYS.md` + `docs/PHASE_6B_JORDAN_GULF_GATEWAYS.md` — `seller_payment_gateways` table (encrypted-at-rest credentials), resolver service, seller self-service CRUD panel, fully wired end-to-end (checkout creation + server-side verify before order placement) for Razorpay, HyperPay, PayTabs, and Tap Payments — the last three chosen by the product owner as the real priority (Jordan/Gulf) over Paystack/Phonepe/further Razorpay work. Not done: Stripe/Paypal/Paystack/Phonepe not wired to the per-seller layer, no admin-level platform-wide settings UI yet for the 3 new gateways (constructors support the fallback; no form to populate it), and per-seller webhook routing (no async webhooks built for any gateway in this app — order completion is synchronous via `place_order()`, matching the existing bar). |
...
| 9 | POS Repair | PARTIAL | `docs/PHASE_9_POS_CART_FIX.md` — re-verified all 4 debt-register bugs against current code: 2 were already fixed in this repo's own Phase 6 (register was stale), the other 2 (`CartService::addToCart()` failing/crashing on new/multi-item carts) fixed here, with the real root cause (return-from-inside-the-loop) turning out broader than the register described. Not done: the brief's fuller POS ask — full-screen responsive UI (Phase 9/10 in the brief's own split), branch-scoped inventory, concurrent-sale stock locking. |
...
| 11 | Platform Commission & Monetization | PARTIAL | `docs/PHASE_11_SUBSCRIPTIONS.md` — `subscription_plans` table + admin CRUD + seller assignment, seeded with 3 placeholder tiers (Basic/Pro/Premium) the admin edits directly (product owner's decision: seed defaults rather than guess real pricing). Commission-rule engine also has a merchant-facing half (Phase 6/6B work). Not done: no billing/payment collection (assignment is manual, nothing charges or auto-expires), no enforcement of plan limits (`commission_rate`/`max_products`) anywhere else in the app yet, no seller-facing subscription view. |
