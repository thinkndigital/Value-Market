...
| 2 | Complete Route & Page Inventory | PARTIAL | `docs/PHASE_2_ROUTE_SWEEP_REPORT.md` — automated sweep of all 298 no-param GET routes across 4 panels (real user, real HTTP kernel). Found 75 real 500s, root-caused every one, fixed the 4 confirmed-safe ones (shared pagination bug), categorized the rest by real-world risk. Param-route sweep now underway incrementally, batch 1 done: 39 admin single-model-CRUD routes, 2 new real bugs found and fixed (`admin/area/edit/{id}` wired to a controller method that never existed; a null-array-offset crash on `admin/sellers/edit/{id}` for any seller with no zipcode set). `tests/Feature/RouteSweepTest.php` + `tests/Feature/Phase2/ParamRouteSweepBatch1Test.php` are permanent regression coverage. Not done: remaining param-route batches (seller/delivery_boy/affiliate, plus admin routes needing richer multi-model fixtures — products, orders, combo products, attributes), and everything past "does it render" (forms/AJAX/search/filters/RTL) per this brief's own fuller checklist. |
