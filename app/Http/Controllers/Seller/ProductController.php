...
class ProductController extends Controller
{
    ...
    public function list()
    {
        ...
        // sends `limit` in real usage so this never hit the actual seller UI, but any direct/API call
        // without it 500'd - matches this controller's own other list methods, which already default it.
    ...
    public function process_bulk_upload(Request $request)
    {
        ...
        if ($type == 'upload') {
            ...
            $handle = fopen($csv, "r");
            try {
                DB::transaction(function () use ($handle, &$temp1, $permissions, $seller_id) {
