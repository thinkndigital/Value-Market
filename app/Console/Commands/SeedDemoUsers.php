...
/**
 * One-shot orchestrator over demo:create-seller / demo:create-affiliate / demo:create-delivery-boy with
 * fixed, documented credentials, so a single command run produces all three demo accounts plus a clean
 * summary of exactly how to log in to each dashboard - no need to invoke and remember three separate
 * commands' worth of options.
 *
 * Each of the three underlying commands validates its own mobile number as unique before creating
 * anything, so running this twice fails cleanly on the second run (an "already taken" validation error per
 * account) rather than silently duplicating or corrupting existing demo data.
 */
class SeedDemoUsers extends Command
{
    protected $signature = 'demo:seed-all {--password= : Password to use for all three accounts (default: Demo@12345)}';

    protected $description = 'Create demo Seller, Affiliate, and Delivery Boy accounts (with placeholder images and sample data) in one step, and print login links for every dashboard.';
