...
// Auth::user()->id instead of this used auth('sanctum')->user()->id in whole file
...
class ChatifyMessenger
{
    ...
    {
        // dd($user_id);
        $where = ['from_id' => auth('sanctum')->user()->id, 'to_id' => $user_id];
        return Message::where($where)
            ->orWhere('from_id', $user_id)->where('to_id', auth('sanctum')->user()->id);
    }
    ...
    {
        // $where = ['from_id' => $user_id, 'seen' => 0, 'to_id' => Auth::user()->id];
        $where = ['from_id' => $user_id, 'seen' => 0, 'to_id' => auth('sanctum')->user()->id];
        Message::Where($where)
            ->update(['seen' => 1]);
        return 1;
    }
    ...
    {
        return Message::where('seen', 0)->where('from_id', '!=', getAuthenticatedUser()->id)->where('to_id', getAuthenticatedUser()->id)->count();
    }
    ...
    {
        $where = ['favorite_id' => $user_id, 'user_id' => auth('sanctum')->user()->id];
    ...
    {
        $url = self::storage()->url(config('chatify.user_avatar.folder') . '/' . $user_avatar_name);
    ...
    {
        $url = self::storage()->url(config('chatify.attachments.folder') . '/' . $attachment_name);
        return preg_replace('/(\/\/)(?=storage)/', '/', $url);
    }
}
