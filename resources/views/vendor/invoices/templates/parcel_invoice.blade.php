...
<body>
    ...
    <div class="content-wrapper" style="min-height: 237.5px;">
        ...
        <!-- Main content -->
        @php
            $settings = app(\App\Services\SettingService::class)->getSettings('system_settings', true);
                    ...
                    <div class="col-md-12">
                        ...
                        @for ($i = 0; $i < count($invoice->seller->custom_fields['seller_ids']); $i++)
                            @php
                                ...
                                // Fetch order charges
                                $order_caharges_data = fetchDetails(\App\Models\OrderCharges::class, [
                                ...
                                $seller_signature = isset($seller_data[0])
                                    ? app(\App\Services\MediaService::class)->getMediaImageUrl($seller_data[0]->authorized_signature, 'SELLER_IMG_PATH')
                                    : null;

                                // Check if seller store data exists and then access the logo
                                $seller_logo = isset($seller_store_data[0])
                                    ? app(\App\Services\MediaService::class)->getMediaImageUrl($seller_store_data[0]->logo, 'SELLER_IMG_PATH')
                                    : null;
                                                ...
                                                <table class="table borderless text-center text-sm">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        @foreach ($invoice->getCustomData() as $row)
                                                            @if ($invoice->seller->custom_fields['seller_ids'][$i] == $row['seller_id'])
                                                                @php
                                                                    $product_variants = app(\App\Services\ProductService::class)->getVariantsValuesById(
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>{{ app(\App\Services\CurrencyService::class)->formateCurrency(formatePriceDecimal($row['price'])) }}<br>
                                                                    </td>
                                                                    <td></td>
                                                                    <td></td>
                                                                    ...
                                                                    <td>{{ app(\App\Services\CurrencyService::class)->formateCurrency(formatePriceDecimal($sub_total)) }}<br>
                                                                    </td>
                                                                </tr>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <th> {{ app(\App\Services\CurrencyService::class)->formateCurrency(formatePriceDecimal($final_sub_total)) }}<br>
                                                            </th>
                                                        </tr>
                                                        <!--  -->
                                                    </tbody>
