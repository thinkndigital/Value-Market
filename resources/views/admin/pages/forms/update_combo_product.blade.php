{{--
    Edit-form counterpart of admin.pages.forms.combo_products (the "add combo product" form) - reached from
    the "Manage Combo Products" list's row-action dropdown (ComboProductController::list()'s $edit_url) via
    ComboProductController::edit(). Mirrors the same pre-fill transformation that update_product.blade.php
    applies over products.blade.php: plain inputs get value="{{ }}", Select2 multi-selects get their existing
    selections pre-rendered as <option selected> before select2() runs, images/files show a preview + hidden
    fallback input, custom fields go through components.product.update_custom_fields, and the multi-language
    tabs pre-fill from the saved translations JSON. The "Attributes" tab is populated client-side by
    custom.js's create_combo_fetched_attributes_html(), triggered by the edit_combo_product_id hidden input
    below (same mechanism update_product.blade.php uses via edit_product_id) - see
    ComboProductController::fetchAttributesById().
--}}
...
@section('content')
    ...
    @endphp
    <x-admin.breadcrumb :title="labels('admin_labels.update_product', 'Update Product')"
        ...
        :breadcrumbs="[
            ...
            ['label' => labels('admin_labels.manage_combo_products', 'Manage Combo Products'), 'url' => route('admin.combo_products.manage_product')],
    ...
    <section class="overview-statistic">
        <div class="">
            ...
            <input type="hidden" class='main_combo_seller_id' value='{{ $data->seller_id }}'>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class='form-control mt-4 combo_seller_id form-select' name='seller_id' id="seller_id">
                                ...
                                @foreach ($sellers as $seller)
                                    ...
                                    @endphp
                                    <option value="{{ $seller->id }}" {{ $data->seller_id == $seller->id ? 'selected' : '' }}>
                    ...
                    <div class="row mt-3 {{ isset($data->has_similar_product) && $data->has_similar_product == '1' ? '' : 'd-none' }}"
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="similar_product_id[]" class="select2 form-select search_admin_combo_product w-100"
                                multiple data-update="1" data-product_id="{{ $data->id }}">
