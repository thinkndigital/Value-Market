{{--
    Edit-form counterpart of seller.pages.forms.combo_products (the "add combo product" form) - reached from
    the seller's "Manage Combo Products" list's row-action dropdown ($edit_url built in
    Seller\ComboProductController::list()) via Seller\ComboProductController::edit(). Mirrors
    admin.pages.forms.update_combo_product (the admin panel's edit-form counterpart of this same resource,
    added in commit de92f4d), scoped to the seller panel: no seller-selection field since the seller is
    implicit (Seller::where('user_id', Auth::id())), x-seller.breadcrumb instead of x-admin.breadcrumb, and
    the seller's own combo-product Select2 widget classes (search_admin_product_for_combo /
    search_admin_digital_product - shared with admin via the same custom.js - and search_seller_combo_product
    for "similar products", matching seller.pages.forms.combo_products). Plain inputs get value="{{ }}",
    Select2 multi-selects get their existing selections pre-rendered as <option selected> before select2()
    runs, images/files show a preview + hidden fallback input, custom fields go through
    components.product.update_custom_fields, and the multi-language tabs pre-fill from the saved translations
    JSON - same transformation update_product.blade.php already applies over products.blade.php for regular
    products. The "Attributes" tab is populated client-side by custom.js's
    create_combo_fetched_attributes_html(), triggered by the edit_combo_product_id hidden input below (same
    mechanism update_product.blade.php uses via edit_product_id) - see
    Seller\ComboProductController::fetchAttributesById(). Note: the description textarea here is named
    "description" (not "pro_input_description" like the create form / update_product.blade.php) because
    Seller\ComboProductController::update() reads $request->description specifically - store() reads
    pro_input_description instead, so the two forms intentionally differ on this one field name.
--}}
...
@section('content')
    @php
        ...
        $isDigital = isset($data->product_type) && $data->product_type == 'digital_product';
        $titleTranslations = json_decode($data->title, true) ?? [];
        $shortDescTranslations = json_decode($data->short_description, true) ?? [];
        $otherImages = json_decode($data->other_images) ?: [];
    @endphp
    <x-seller.breadcrumb :title="labels('admin_labels.update_product', 'Update Product')"
        ...
        :breadcrumbs="[
            ...
            ['label' => labels('admin_labels.manage_products', 'Manage Products'), 'url' => route('seller.combo_products.manage_product')],
            ['label' => labels('admin_labels.update_product', 'Update Product')],
        ]" />

    <section class="overview-statistic">
        <div class="">
            <input type="hidden" name="edit_combo_product_id" value="{{ $data->id }}">
            ...
            <form action="{{ route('seller.combo_products.update', $data->id) }}" class="submit_form"
                ...
                <div class="card p-5">
                    ...
                    <div class="row mt-4">
                        <div class="col-md-6">
                            ...
                            <select class='form-control mt-4' name='product_type_in_combo' id="product_type_menu" disabled>
                                <option value="physical_product" {{ !$isDigital ? 'selected' : '' }}>
                                    {{ labels('admin_labels.physical_product', 'Physical Product') }}
                                </option>
                                <option value="digital_product" {{ $isDigital ? 'selected' : '' }}>
                ...
                <div class="card p-5 mt-4">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control" id="short_description"
                                        ...
                                        name="short_description">{{ $shortDescTranslations['en'] ?? '' }}</textarea>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <textarea class="form-control" id="translated_short_description_{{ $lang->code }}"
                                                placeholder="Product Short Description"
                                                name="translated_product_short_description[{{ $lang->code }}]">{{ $shortDescTranslations[$lang->code] ?? '' }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            @endif
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-6 physical_product_in_combo {{ $isDigital ? 'd-none' : '' }}">
                            <div class="form-group">
                                ...
                                <select name="physical_product_variant_id[]"
                                    class="select2 form-select search_admin_product_for_combo w-100" multiple>
                                    @if (!$isDigital)
                                        @foreach ($product_details as $product_detail)
                                            <option value="{{ $product_detail['id'] }}" selected>
                                                {{ $product_detail['name'] }}{{ !empty($product_detail['variant_name']) ? ' - ' . $product_detail['variant_name'] : '' }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
                        ...
                        <div class="col-md-6 digital_product_in_combo {{ $isDigital ? '' : 'd-none' }}">
                            <div class="form-group">
                                <label class="form-label" for="name">
                                ...
                                <select name="digital_product_id[]"
                                    ...
                                    @if ($isDigital)
                                        @foreach ($product_details as $product_detail)
                                            <option value="{{ $product_detail['product_id'] }}" selected>
                                                {{ $product_detail['name'] }}
                                            </option>
                                        @endforeach
                                    @endif
                    ...
                    <div class="row mt-3 {{ isset($data->has_similar_product) && $data->has_similar_product == '1' ? '' : 'd-none' }}"
                        id="similar_product">
                        <div class="col-md-12">
                            ...
                            <select name="similar_product_id[]" class="select2 form-select search_seller_combo_product w-100"
                                multiple>
                                @foreach ($combo_product_details as $combo_product_detail)
                                    @php
                                        $similar_title = json_decode($combo_product_detail['title'], true);
                                        $similar_title = $similar_title[$language_code] ?? ($similar_title['en'] ?? '');
                                    @endphp
                                    <option value="{{ $combo_product_detail['id'] }}" selected>{{ $similar_title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="pro_input_tax[]" class="tax_list form-select w-100" multiple>
                                    ...
                                    @foreach ($tax_name as $row)
                                        <option value="{{ $row->id }}" @if (in_array($row->id, $taxes)) selected @endif>
                ...
                <div class="card p-5 mt-4 combo_product_quantity_and_others {{ $isDigital ? 'd-none' : '' }}">
                        ...
                        <div class="col-md-6 total_allowed_quantity">
                            <div class="form-group">
                                ...
                                <input type="number" class="col-md-12 form-control" name="total_allowed_quantity"
                                    value="{{ $data->total_allowed_quantity }}" min=1 placeholder="Total Allowed Quantity">
                            </div>
                        </div>
                        <div class="col-md-6 minimum_order_quantity">
                            <div class="form-group">
                                ...
                                <input type="number" class="col-md-12 form-control" name="minimum_order_quantity" min="1"
                                    value="{{ $data->minimum_order_quantity ?: 1 }}" placeholder="Minimum Order Quantity">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 quantity_step_size">
                            <div class="form-group">
                                ...
                                <input type="number" class="col-md-12 form-control" name="quantity_step_size" min="1"
                                    value="{{ $data->quantity_step_size ?: 1 }}" placeholder="Quantity Step Size">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card p-5 mt-4 combo_delivery_and_shipping_setting {{ $isDigital ? 'd-none' : '' }}">
                        ...
                        <div class="col-md-6 ">
                            <div class="form-group">
                                ...
                                <select class="form-select form-select" name="deliverable_type" id="deliverable_type">
                                    <option value="0" {{ $data->deliverable_type == '0' ? 'selected' : '' }}>None
                                    </option>
                                    <option value="1" class="combo_all_deliverable_type" {{ $data->deliverable_type == '1' ? 'selected' : '' }}>
                                        All</option>
                                    <option value="2" {{ $data->deliverable_type == '2' ? 'selected' : '' }}>specific
                                    </option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="deliverable_zones[]" class="search_seller_zone form-select w-100" multiple
                                    id="deliverable_zones" {{ $data->deliverable_type == '2' || $data->deliverable_type == '3' ? '' : 'disabled' }}>
                                    @if ($data->deliverable_type == '2' || $data->deliverable_type == '3')
                                        @php
                                        ...
                                        @foreach ($zone_names as $row)
                                            <option value="{{ $row->id }}" {{ in_array($row->id, $zones) ? 'selected' : '' }}>
                                                ...
                                                |
                                                Serviceable Cities: {{ implode(', ', $row->serviceable_city_names) }} |
                                                Serviceable Zipcodes: {{ implode(', ', $row->serviceable_zipcodes) }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select class='form-control shiprocket_type form-select' name="pickup_location"
                                    ...
                                    @foreach ($shipping_data as $val)
                                        <option value="{{ $val->pickup_location }}" {{ $val->pickup_location == $data->pickup_location ? 'selected' : '' }}>
                                            {{ $val->pickup_location }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" value="{{ $data->minimum_free_delivery_order_qty }}"
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" value="{{ $data->delivery_charges }}" min=1
                                ...
                                <div class="tab-pane fade active show" id="general-settings" role="tabpanel"
                                    ...
                                    <div id="product-general-settings">
                                        <div id="general_price_section" class="">
                                                    ...
                                                    <div class="col-md-12">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="number" name="simple_price"
                                                                ...
                                                                value="{{ $data->price }}" min="0.01" step="0.01">
                                                        </div>
                                                    </div>
                                                    <div class="col-md-12">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="number" name="simple_special_price"
                                                                ...
                                                                value="{{ $data->special_price }}" min="0" step="0.01">
                                                        </div>
                                                    </div>
                                                </div>

                                                <div class="col-md-6 {{ $isDigital ? 'd-none' : '' }}">
                                                            ...
                                                            <div class="col-6">
                                                                <div class="form-group">
                                                                    ...
                                                                    <input type="number" class="form-control" name="height"
                                                                        ...
                                                                        value="{{ $data->height }}" step="0.01" min=0>
                                                            ...
                                                            <div class="col-6">
                                                                <div class="form-group">
                                                                    ...
                                                                    <input type="number" class="form-control" name="breadth"
                                                                        ...
                                                                        value="{{ $data->breadth }}" step="0.01" min=0>
                                                                </div>
                                                            </div>
                                                            <div class="col-6">
                                                                <div class="form-group">
                                                                    ...
                                                                    <input type="number" class="form-control" name="length"
                                                                        ...
                                                                        value="{{ $data->length }}" step="0.01" min=0>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div
                                                class="form-group  simple_stock_management {{ $isDigital ? 'd-none' : '' }}">
                                                <div class="col">
                                                    <input type="checkbox" name="simple_stock_management_status"
                                                        ...
                                                        {{ !is_null($data->availability) ? 'checked' : '' }}>
                                                    <span
                                        ...
                                        <div
                                            class="simple-product-level-stock-management {{ $isDigital ? 'd-none' : '' }} {{ is_null($data->availability) ? 'collapse' : '' }}">
                                                ...
                                                <div class="col col-xs-4 col-md-4">
                                                    <div class="form-group">
                                                        ...
                                                        <select type="text" name="simple_product_stock_status"
                                                            ...
                                                            <option value="1" {{ $data->availability == '1' ? 'selected' : '' }}>
                                                                In Stock</option>
                                                            <option value="0" {{ $data->availability == '0' ? 'selected' : '' }}>
                                                                Out Of Stock</option>
                                                        </select>
                                                    </div>
                                                </div>
                                    ...
                                    <div id="digital_product_setting" class="{{ $isDigital ? '' : 'collapse' }}">
                                            ...
                                            <div class="col-md-6 d-flex">
                                                ...
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" type="checkbox" name="download_allowed"
                                                        id="download_allowed" {{ $data->download_allowed == '1' ? 'checked' : '' }}>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col-xs-6 {{ $data->download_allowed == '1' ? '' : 'collapse' }}"
                                                id='download_type'>
                                                ...
                                                <select class='form-control form-select' name="download_link_type"
                                                    ...
                                                    <option value='self_hosted' {{ $data->download_type == 'self_hosted' ? 'selected' : '' }}>
                                                        Self Hosted</option>
                                                    <option value='add_link' {{ $data->download_type == 'add_link' ? 'selected' : '' }}>
                                                        Add Link</option>
                                                </select>
                                            </div>
                                            <div class="col-md-6 {{ $data->download_type == 'add_link' ? '' : 'd-none' }}"
                                                ...
                                                <input type="url" class='form-control' name='download_link'
                                                    id='download_link'
                                                    value="{{ $data->download_type == 'add_link' ? $data->download_link : '' }}"
                                                    placeholder="Paste digital product link or URL here">
                                            </div>
                                            <div class="form-group {{ $data->download_type == 'self_hosted' ? '' : 'd-none' }} mt-5"
                                                ...
                                                <div class="row mt-3 image-upload-section">
                                                    @if ($data->download_type == 'self_hosted' && $data->download_link)
                                                        <div
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <div class="container-fluid row mt-3 image-upload-section">
                                    <div class="col-12 p-3 mb-5 bg-white rounded m-4 text-center grow image">
                                        ...
                                        @foreach ($otherImages as $row)
                                            <div class="col-6">
                                                <div class="bg-white grow image rounded shadow text-center m-2">
                                                    ...
                                                    <a href="javascript:void(0)" class="delete-img" data-id="{{ $data->id }}"
                                                        data-field="other_images" data-img="{{ $row }}"
                                                        data-table="combo_products" data-path="{{ $row }}" data-isjson="true">
                                                        <span class="btn btn-block bg-gradient-danger text-danger btn-xs">
                    ...
                    <div class="row mt-4">
                        <div class="col-md-12">
                            ...
                            <textarea class="form-control mt-2 d-none mb-2 custom_description_prompt"
                                ...
                                placeholder="Enter your own AI prompt (optional)..."></textarea>
                            {{-- Seller\ComboProductController::update() reads $request->description (unlike store(),
                            which reads pro_input_description) - see the note at the top of this file. --}}
                            <textarea id="pro_input_description" name="description" class="form-control addr_editor"
                                placeholder="Place some text here">{{ $data->description }}</textarea>
                        </div>
                    </div>
                </div>
